...
const App = () => {
  ...
  const getAllPosts = async () => {
    try {
      const resp = await axios.get("http://localhost:5002/api/v1/post")
  ...
  const delete_post = async (postId) => {
    ...
    try {
      const resp = await axios.delete(`http://localhost:5002/api/v1/post/${postId}`)
      alert("post deleted")
      getAllPosts()
  ...
  const edit_post = async (postId, title, description) => {
    ...
    try {
      const resp = await axios.put(`http://localhost:5002/api/v1/post/${postId}`, {
        title: updatedTitle,
          ...
          return (
            <div key={index} className='border-1 p-2 flex flex-col gap-2 rounded-lg'>
              ...
              <div className='flex gap-2'>
                <button
                  onClick={() => edit_post(singlePost.id, singlePost.title, singlePost.description)}
                ...
                <button
                  onClick={() => delete_post(singlePost.id)}
